import { python } from "https://cdn.jsdelivr.net/npm/@codemirror/lang-python@6.1.3/dist/index.min.js";
import { html } from "https://cdn.jsdelivr.net/npm/@codemirror/lang-html@6.4.5/dist/index.min.js";
import { javascript } from "https://cdn.jsdelivr.net/npm/@codemirror/lang-javascript@6.2.1/dist/index.min.js";
import { css } from "https://cdn.jsdelivr.net/npm/@codemirror/lang-css@6.2.1/dist/index.min.js";
import { yaml } from "https://cdn.jsdelivr.net/npm/@codemirror/lang-yaml@6.0.0/dist/index.min.js";
import { sql } from "https://cdn.jsdelivr.net/npm/@codemirror/lang-sql@6.5.2/dist/index.min.js";
import { EditorView, basicSetup } from "https://cdn.jsdelivr.net/npm/codemirror@6.0.1/dist/index.min.js";
import { dracula } from "https://cdn.jsdelivr.net/npm/@codemirror/theme-dracula@0.21.0/dist/index.min.js";
...
async function displayRepos() {
    ...
    for (const repo of reposToShow) {
        ...
        container.innerHTML = `
            <h3>${repo.name}</h3>
            <div id="readme-${repo.name}" class="readme-container"></div>
            <button class="btn btn-primary readme-toggle" onclick="ToggleReadMe('${repo.name}', '${repo.full_name}')">{{ i18n.t('TOGGLE_README', lang) }}</button>
            <div class="chart-row">
                <div class="chart-container">
                    <h4>{{ i18n.t('VIEWS', lang) }}</h4>
                    <canvas id="views-chart-${repo.name}"></canvas>
                </div>
                <div class="chart-container">
                    <h4>{{ i18n.t('CLONES', lang) }}</h4>
                    <canvas id="clones-chart-${repo.name}"></canvas>
                </div>
            </div>
        `;
        ...
        const viewLabels = completeViews.map(v => formatDate(v.timestamp));
        const viewData = completeViews.map(v => v.count);
        const viewTotal = completeViews.reduce((sum, v) => sum + v.count, 0);
        createChart(`views-chart-${repo.name}`, '{{ i18n.t('VIEWS', lang) }}', viewData, viewLabels, viewTotal);

        const cloneLabels = completeClones.map(v => formatDate(v.timestamp));
        const cloneData = completeClones.map(v => v.count);
        const cloneTotal = completeClones.reduce((sum, v) => sum + v.count, 0);
        createChart(`clones-chart-${repo.name}`, '{{ i18n.t('CLONES', lang) }}', cloneData, cloneLabels, cloneTotal);
    }
...
(async () => {
    ...
    if (allRepos.repositories.length === 0) {
        showInfoBox();
        document.getElementById('loading').style.display = 'none';
    } else {
        await displayRepos();
...
function initializeEditor(content = '', mode = 'text') {
    ...
    const language = modeMap[mode] || null;
...
async function fetchFileTree() {
    try {
        const response = await fetch(`/{{ lang }}/files/list`);
        ...
        alert('{{ i18n.t('ERROR_FETCHING_FILES', lang) }}');
    }
}

function renderFileTree(files, container, prefix = '') {
    container.innerHTML = '';
    files.forEach(file => {
        const div = document.createElement('div');
        div.className = `file-tree-item ${file.type}`;
        const indent = prefix ? `${prefix}  ` : '';
        div.innerHTML = `
            ${indent}${file.type === 'directory' ? '📁' : '📄'} 
            <span class="file-name">${file.name}</span>
        `;
        if (file.type === 'file') {
            div.querySelector('.file-name').addEventListener('click', () => loadFile(file.path));
        } else {
            div.querySelector('.file-name').addEventListener('click', () => toggleDirectory(div, file));
        }
        container.appendChild(div);
        if (file.children) {
            const childContainer = document.createElement('div');
            childContainer.className = 'file-tree-child';
            childContainer.style.display = 'none';
            div.appendChild(childContainer);
            renderFileTree(file.children, childContainer, indent);
        }
    });
}

function toggleDirectory(div, file) {
    const childContainer = div.querySelector('.file-tree-child');
    if (childContainer) {
...
async function loadFile(path) {
    try {
        const response = await fetch(`/{{ lang }}/files/read`, {
            method: 'POST',
        ...
        alert('{{ i18n.t('ERROR_LOADING_FILE', lang) }}');
    }
}
...
document.getElementById('create-file-form').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`/{{ lang }}/files/create`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ path })
        });
        ...
        alert('{{ i18n.t('ERROR_CREATING_FILE', lang) }}');
    }
});
...
document.getElementById('save-file').addEventListener('click', async () => {
    ...
    try {
        ...
        const response = await fetch(`/{{ lang }}/files/save`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        alert('{{ i18n.t('ERROR_SAVING_FILE', lang) }}');
    }
});

document.getElementById('delete-file').addEventListener('click', async () => {
    if (!currentFilePath) return;
    if (!confirm('{{ i18n.t('CONFIRM_DELETE', lang) }}')) return;
    try {
        const response = await fetch(`/{{ lang }}/files/delete`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        alert('{{ i18n.t('ERROR_DELETING_FILE', lang) }}');
    }
});

document.querySelector('.tab[data-tab="files"]').addEventListener('click', () => {
    fetchFileTree();
});
